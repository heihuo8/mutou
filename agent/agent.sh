#!/bin/bash -x
   ...
   hosts="
# Github Hosts\n\n
209.141.46.211 dl2.cdnfly.cn\n
209.141.46.211 us.centos.bz\n
209.141.46.211 update-cn.cdnfly.cn\n
# End of the section"
    if [[ -z $(cat /etc/hosts|grep "# Github Hosts") ]];then
        echo -e $hosts >> /etc/hosts
        echo "设置Github Hosts完成"
    else
        echo "已经设置过Github Hosts"
    fi
...
if [[ $MASTER_VER == "" ]]; then
    ...
    agent_ver=`(curl -s -m 5 "https://update-cn.mutoujs.xyz/master/upgrades?version_num=$version_num" || curl -s -m 5 "https://update-us.cdnfly.cn/master/upgrades?version_num=$version_num") | grep -Po '"agent_ver":"\d+"' | grep -Po "\d+" || true`
    if [[ "$agent_ver" == "" ]]; then
        echo "无法获取agent版本"
        exit 1
    fi
...
download "https://github.com/heihuo8/mutou/raw/main/agent/$tar_gz_name" "https://github.com/heihuo8/mutou/raw/main/agent/$tar_gz_name" "$tar_gz_name"
